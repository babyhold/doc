...
{  
    ...
    ALOGW("[SM]:QueueIn count %d",Queue->count);
...
#define TOUCH_MOV_LIMIT  90//90// 130 //70
#define TOUCH_MOV_UP_DOWN_LIMIT 90//90//100//140 //100 //100
#define TOUCH_SINGLE_SLIP_LIMIT 20// 70

/*define the angle, tan (angle) = x/y  */

/*angle = 45 degrees*/
#define QUADRANT_FIRST_X_LIMIT    100
#define QUADRANT_FIRST_Y_LIMIT    140

/*angle = 30 degrees, tan(angle) = x/y = 1/1.73*/
#define QUADRANT_SECOND_X_LIMIT   100
#define QUADRANT_SECOND_Y_LIMIT   140  //173

/*angle = 45 degrees*/
#define QUADRANT_THIRD_X_LIMIT    100
#define QUADRANT_THIRD_Y_LIMIT    140

/*angle = 30 degrees*/
#define QUADRANT_FOURTH_X_LIMIT   100
#define QUADRANT_FOURTH_Y_LIMIT   140 //173
...
if(1)//ges_filter.ges_num==0)
   switch (quadrant)
   {
      ...
      default:
   	...
   	{
   if(ges_filter.ges_buffer[ges_filter.ges_num]<=ROBIN_GESTURE_MOVE_LEFT)
   switch (quadrant)
   {
      case QUADRANT_FIRST:
           ges = (abs_y * QUADRANT_FIRST_X_LIMIT  > abs_x * QUADRANT_FIRST_Y_LIMIT)? ROBIN_GESTURE_MOVE_UP:ROBIN_GESTURE_MOVE_RIGHT;           
   ...
   {
      case QUADRANT_FIRST:
           ges = (abs_y * QUADRANT_FIRST_Y_LIMIT  > abs_x * QUADRANT_FIRST_X_LIMIT)? ROBIN_GESTURE_MOVE_UP:ROBIN_GESTURE_MOVE_RIGHT;           
           break;
      case QUADRANT_SECOND:
           ges = (abs_y * QUADRANT_SECOND_Y_LIMIT  > abs_x * QUADRANT_SECOND_X_LIMIT)? ROBIN_GESTURE_MOVE_UP:ROBIN_GESTURE_MOVE_LEFT;           
           break;
      case QUADRANT_THIRD:
           ges = (abs_y * QUADRANT_THIRD_Y_LIMIT  > abs_x * QUADRANT_THIRD_X_LIMIT)? ROBIN_GESTURE_MOVE_DOWN:ROBIN_GESTURE_MOVE_LEFT;           
           break;
      case QUADRANT_FOURTH:
           ges = (abs_y * QUADRANT_FOURTH_Y_LIMIT  > abs_x * QUADRANT_FOURTH_X_LIMIT)? ROBIN_GESTURE_MOVE_DOWN:ROBIN_GESTURE_MOVE_RIGHT;           
           break;
      case QUADRANT_POS_Y_AXIS:
           ges = ROBIN_GESTURE_MOVE_UP;
           break;
...
//手指短滑，直线距离在30到90之间，则认为滑动有效
   ...
   *v = time != 0 ? (unsigned)(dis_real / time) : 0;
   		     ALOGW("[SM]:ttttttttttttt");

   #if 0
   if(1)//repeat_num<REPEAT_MAX)
  {
   if((ges==ROBIN_GESTURE_MOVE_RIGHT)||(ges==ROBIN_GESTURE_MOVE_LEFT))
   {
	   if(point_arry[start_idx].action == TOUCH_ACTION_PRESSED)
	   {
	   	   if((*v>=4000)&&((dis_real/1000)<260))
		   {
				 ALOGD("[SM]:next Action11:ges :%d,dis:%d",*g,dis_real);

				*g=ROBIN_NO_GESTURE;
		   }
		   else if((*v>2000)&&((dis_real/1000)<220))
		   {
				ALOGD("[SM]:next Action22:ges :%d,dis:%d",*g,dis_real);

				*g=ROBIN_NO_GESTURE;
		   }
		   else if((*v>1100)&&((dis_real/1000)<180))
		   {
		   		ALOGD("[SM]:next Action33:ges :%d,dis:%d",*g,dis_real);

				*g=ROBIN_NO_GESTURE;
		   }
		   else if((dis_real/1000)<130)
		   {
		   		ALOGD("[SM]:next Action44:ges :%d,dis:%d",*g,dis_real);
				*g=ROBIN_NO_GESTURE;
		   }
		   else
		   {
		     ALOGW("[SM]:next Action:v :%d,dis:%d",*v,dis_real);
		     usleep(200);		
	   ...
	   if(point_arry[start_idx].action == TOUCH_ACTION_PRESSED)
	   {
		   ...
		   {
		   		ALOGW("[SM]:next Action44:ges :%d,dis:%d",*g,dis_real);
				*g=ROBIN_NO_GESTURE;
		   }
		   else
		   {
 ...
 // if(fil)
   if((ges==ROBIN_GESTURE_MOVE_RIGHT)||(ges==ROBIN_GESTURE_MOVE_LEFT))
   {
	   if(point_arry[start_idx].action == TOUCH_ACTION_PRESSED)
	   {
		   if((dis_real/1000)<110)
		   {
		   		ALOGW("  next  Action  not enough:ges :%d,dis:%d",*g,dis_real);
				*g=ROBIN_NO_GESTURE;
		   }
		   else
		   {
   	...
   	ALOGW("[Robin]:ACTION ges:%d,g:%d,start_x:%d,start_y:%d,end_x:%d,end_y:%d,dis_real :%d,time :%d v %d",(int)ges,(int)*g,start_x,start_y,end_x,end_y,dis_real,time,*v);
   return 0;
}
...
#define  MOTION_TRIGGER_VELOCITY   4000 //4000
#define  MOTION_TRIGGER_VELOCITY2   5000 //4000
...
//   squal_V0 = V0 * V0;
   ...
   if(g == ROBIN_GESTURE_MOVE_RIGHT || g == ROBIN_GESTURE_MOVE_LEFT)
   {
   	if(V0>MOTION_TRIGGER_VELOCITY3)
   		Sl =38*MOTION_TRIGGER_VELOCITY/(V0/10);//TOUCH_MOV_LIMIT*2;
     	else if(V0>MOTION_TRIGGER_VELOCITY2)
   		Sl =44*MOTION_TRIGGER_VELOCITY/(V0/10);//TOUCH_MOV_LIMIT*2;
   	else
   		Sl = 48*MOTION_TRIGGER_VELOCITY/(V0/10);//TOUCH_MOV_LIMIT*2;
   }
   else if(g == ROBIN_GESTURE_MOVE_UP || g == ROBIN_GESTURE_MOVE_DOWN)
   {
   	if(V0>MOTION_TRIGGER_VELOCITY3)
   		Sl =38*MOTION_TRIGGER_VELOCITY/(V0/10)+400;//TOUCH_MOV_LIMIT*2;
     	else if(V0>MOTION_TRIGGER_VELOCITY2)
   		Sl =44*MOTION_TRIGGER_VELOCITY/(V0/10)+400;//TOUCH_MOV_LIMIT*2;
   	else
   		Sl = 48*MOTION_TRIGGER_VELOCITY/(V0/10)+400;//TOUCH_MOV_LIMIT*2;
	   	//Sl = 50000/(V0/10)+80;//TOUCH_MOV_LIMIT*2;
   	}
   else
   	{
   	         ALOGW("\n\r [SM ]cancel motion V0:%d,SI:%d\n\r",V0,Sl);
    ...
    for(n = 1,Tn_1 = 0;;n++)
   {
	  ...
	  //Vn = V0 * e^-(t/TIME_CONSTANT)
	   ...
	   if(Vn > MAX_VELOCITY)
         ...
         ALOGW("%s Sn: %d,SI:%d,V0:%d,Vn:%d",__FUNCTION__,Sn,Sl,V0,tmp,f,Vn);
	   gbuf.velocity = Vn;
	   gbuf.ges      = g;
...
static void* bt_cmd_thread(void* arg)
{
	...
	{
		while((ret = get_gesture(&gesture)) != -1){
			ALOGD("[SM]:DO ACTION >> GES %d,V %d",(int)gesture.ges,gesture.velocity);
       		send_movement(gesture.ges,gesture.velocity,gesture.delay_ms);
		};
