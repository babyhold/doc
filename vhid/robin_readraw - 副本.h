...
#define TOUCH_MAX_X      380
#define TOUCH_MAX_Y      380
...
#define TOUCH_SINGLE_SLIP_LIMIT 30
...
static int save_gesture(gesture_t ges,unsigned int v,unsigned int delay_ms)
{
    ...
    ALOGD("[Robin]:ACTION g.ges %d, g.velocity %d",ges,v);
...
#define  MOTION_TRIGGER_VELOCITY   1000 //4000
#define  MAX_VELOCITY       5000

#define  TIME_CONSTANT     400//325
...
*/
   ...
   // ALOGD("%s ges_filter.ges_num: %d,V0:%d ",__FUNCTION__,ges_filter.ges_num,V0);
...
static int robin_read_hidraw_test( char buf[10])
{
	...
	touch_action.y      = ((buf[4] & 0x03) << 8) | buf[5];
      ...
      //给耐薄分析工具使用
    	v = 0;
	if(touch_action.action == TOUCH_ACTION_PRESSED && touch_action.x <= TOUCH_MAX_X && touch_action.y <= TOUCH_MAX_Y &&
	...
	abs_x = touch_action.x > first_bt_point.x ? touch_action.x - first_bt_point.x:last_bt_point.x - touch_action.x;
	abs_y = touch_action.y > first_bt_point.y ? touch_action.y - first_bt_point.y:last_bt_point.y - touch_action.y;
	dis = sqrt(abs_x * abs_x + abs_y * abs_y);
	
	ALOGD("[POINTS_INFO_TAG] ACTION:%d X_LOCATION:%d Y_LOCATION:%d V_VALUE:%d DIS:%d\n",(unsigned int)touch_action.action,
			touch_action.x,
			touch_action.y,
			v,dis);
